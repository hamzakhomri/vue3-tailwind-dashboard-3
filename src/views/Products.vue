<template>
  ...
  <div class="flex justify-between">
      <h2 class="text-gray-500 p-2">Categories : {{ ListProduct.length }}</h2>
  ...
  <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
    ...
    <tbody>
      <tr v-for="product in ListProduct" :key="product.idProductproduct" 
      class="bg-gray-100 hover:bg-blue-100 border border-indigo-600 dark:bg-gray-800 dark:hover:bg-gray-600   dark:border-gray-700 w-96">
        ...
        <td class="py-4 px-6">
          {{ product.nameProductCategory }}
        </td>
        <td class="py-4 px-6">
          {{ product.createdProductCategory }}
        </td>
        <td class="py-4 px-6">
          {{ product.modifiedProductCategory }}
        </td>
        ...
        <td class="py-4 px-6 text-right">
          <button @click="confirmUpdate(product.idProductCategory,product.nameProductCategory)">Modifier</button>
        </td>
      </tr>
    ...
    computed: {
              ListProduct() {
                if (this.searchCategoryID !== '') {
                  return this.ListProduct; // Return all categories when searchCategoryID is empty
                } else {
                  return this.ListProduct.filter(product => 
                  product.nameProductCategory.includes(this.searchCategoryName) ||
                  product.idProductCategory === this.searchCategoryID
                  );
                }
          },
          //========= Search
          ListProduct() {
              if (this.searchCategoryID !== '') {
                return this.ListProduct.filter(product =>
                  product.idListProduct === this.searchCategoryID
                );
              }
              ...
              return this.ProductCategory.filter(product => 
                product.createdProductCategory === searchDates
              );
            }
            if (this.searchDateModification!=='') {
              ...
              return this.ProductCategory.filter(product => 
                product.modifiedProductCategory === searchDates
              );
            }
